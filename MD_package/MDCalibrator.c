...
MDCalibratorCalculateTime(MDCalibrator *inCalib, MDTickType inTick)
{
	...
	return time_before + (MDTimeType)floor(0.5 + (inTick - tick_before) * floor(60000000.0 / tempo) / timebase);
}
...
MDCalibratorTimeToTick(MDCalibrator *inCalib, MDTimeType inTime)
{
	...
	timebase = MDSequenceGetTimebase(inCalib->parent);
    //  floor(60000000.0/tempo): MIDI integer tempo
	return tick_before + (MDTickType)floor(0.5 + (double)(inTime - time_before) * ((double)timebase / floor(60000000.0 / MDCalibratorGetTempo(inCalib))));
}
