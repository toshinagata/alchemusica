...
long *outMeasure, long *outBeat, long *outTick)
{
	...
	if (tickPerBeat == 0 || beatPerMeasure == 0) {
		...
		if (eptr == NULL) {
			/*  eptr == NULL ‚Ìê‡Adata_before.bar = 1, tick_before = 0 ‚Æ‚µ‚ÄŒvZ‚·‚é  */
			theTickBefore = 0;
			theBarBefore = 1;
		} else {
...
MDCalibratorBackward(MDCalibrator *inCalib)
{
	...
	switch (inCalib->kind) {
		...
		//	inCalib->data_after.bar = inCalib->data_before.bar;
			...
			/*  after ˆÊ’u‚Ì¬ß”‚ª data_after.bar ‚Æ“™‚µ‚­‚È‚é‚æ‚¤‚É data_before.bar ‚ğ’²ß‚·‚é  */
